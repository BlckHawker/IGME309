...
void MyCamera::CalculateView(void)
{
	...
	//get the rotation based on the pitch (x) and yaw (y) relative to the right and up vectors
	quaternion q = glm::angleAxis(m_v3PitchYawRoll.x, m_v3Rightward);
	q *= glm::angleAxis(m_v3PitchYawRoll.y, m_v3Upward);

	//rotate fowards based on rotation
	m_v3Forward = glm::rotate(q, m_v3Forward);

	//update target and right vectors
	m_v3Target = m_v3Forward + m_v3Position;
	m_v3Rightward = glm::cross(m_v3Forward, m_v3Upward);

	//fix sensitivity of cameras
	m_m4View = glm::lookAt(m_v3Position, m_v3Target, m_v3Upward);
	m_v3PitchYawRoll *= 0.3f;
}
